...
class OULADDataset(StandardDataset):
    ...
    def __init__(self, df = None, domain:str = 'all',
        ...
        """
            domain : 'all' for complete dataset, 'stem' for module FFF with no duplicate students, 'social' for module BBB with no duplicate students
        ...
        if df is None :
            try :
                ...
                print("\nand place them, as-is, in the directory 'dataset/OULAD'")
                import sys
                sys.exit(1)
        ...
        #Retrieve wanted students and preprocess to add features related to VLE
        ...
        if domain != 'all' :
            ...
            #Remove duplicated students, keep last occurence
            for id in df.index :
                df_rest = df.loc[id:,:]
            ...
            # WARNING may contain info about same student for different presentation
    ...
    def to_df_multiclass(self,label_multi: str) :
        # Return the dataset content as a Dataframe with the additional feature 'label multiclass' holding a numerical label corresponding unbinarized final_result
