...
def plot_all(data_list, bias_list, preproc_list, postproc_list, model_list, blinding_list, all_bias, path_start) :
                ...
                for model in model_list :
                    for blind in blinding_list :
                        ...
                        if len(postproc_list) == 0 :
                            ...
                            """
                            plot_by_bias(metricsBiased_for_plot, all_bias, bias, biased_test=True, plot_style='FILLED_STDEV', title='',path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'_'+preproc+'_'+model+visibility+'_byBias_BiasedTest', display=False)
                            plot_by_bias(metrics_for_plot, all_bias, bias, biased_test=False, plot_style='FILLED_STDEV',title='', path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'_'+preproc+'_'+model+visibility+'_byBias_unbiasedTest', display=False)
                            ...
                            for postproc in postproc_list :
                                path = path_start+"Results/"+ds+'_'+bias+'_'+preproc+'_'+model+visibility+'_'+postproc
                                print(path)
                                ...
                                """ Version with title
                                plot_by_bias(metrics_BVFT, all_bias, bias, biased_test=False, plot_style='FILLED_STDEV', title='Metric values wrt train set bias level\n ('+bias+' bias, '+model+visibility+', '+postproc+', biased postproc & fair test set)',path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'__'+model+visibility+'_'+postproc+'-BiasedValidFairTest_byBias_', display=False)
                                plot_by_bias(metrics_BVBT, all_bias, bias, biased_test=True, plot_style='FILLED_STDEV',title='Metric values wrt train set bias level\n ('+bias+' bias, '+model+visibility+', '+postproc+', biased postproc & biased test set)', path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'__'+model+visibility+'_'+postproc+'-BiasedValidBiasedTest_byBias_', display=False)
                                plot_by_bias(metrics_FVFT, all_bias, bias, biased_test=False, plot_style='FILLED_STDEV',title='Metric values wrt train set bias level\n ('+bias+' bias, '+model+visibility+', '+postproc+', fair postproc & fair test set)', path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'__'+model+visibility+'_'+postproc+'-FairValidFairTest_byBias_', display=False)
                                """
                                plot_by_bias(metrics_BVFT, all_bias, bias, biased_test=False, plot_style='FILLED_STDEV', title='',path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'__'+model+visibility+'_'+postproc+'-BiasedValidFairTest_byBias_', display=False)
                                plot_by_bias(metrics_BVBT, all_bias, bias, biased_test=True, plot_style='FILLED_STDEV',title='', path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'__'+model+visibility+'_'+postproc+'-BiasedValidBiasedTest_byBias_', display=False)
                                plot_by_bias(metrics_FVFT, all_bias, bias, biased_test=False, plot_style='FILLED_STDEV',title='', path_start='Code/ControlledBiasPrivate/plots/'+ds+'_'+bias+'__'+model+visibility+'_'+postproc+'-FairValidFairTest_byBias_', display=False)
...
def plot_tradeoff(retrieval_path:str, metric:str, dataset_list:list[str], model_list:list[str], bias:str,  all_bias:list[float], biased_test:bool,
    ...
    for ds in dataset_list :
        for model in model_list :
            ...
            ax.plot(indices-3*fuzzy,metrics_all[ds][model]['reweighting'][metric]['mean'], label = "Reweighing", linestyle="--", marker="s", markeredgewidth=0, c=colors['reweighting'], alpha=1)
            ax.plot(indices-2*fuzzy,metrics_all[ds][model]['massaging'][metric]['mean'], label = "Massaging", linestyle="--", marker="D", markeredgewidth=0, c=colors['massaging'], alpha=1)
            ax.plot(indices-1*fuzzy,metrics_all[ds][model]['FTU'][metric]['mean'], label = "FTU", linestyle="--", marker="p", markeredgewidth=0, c=colors['FTU'], alpha=1)
            ...
            ax.plot(indices+2*fuzzy,metrics_all[ds][model]['calEqOddsProc'][metric]['mean'], label = "CEO", linestyle="--", marker="h", markeredgewidth=0, c=colors['calEqOddsProc'], alpha=1)
            ax.plot(indices+3*fuzzy,metrics_all[ds][model]['ROC-SPD'][metric]['mean'], label = "ROC-SPD", linestyle="--", marker="d", markeredgewidth=0, c=colors['ROC-SPD'], alpha=1)
            ax.plot(indices+4*fuzzy,metrics_all[ds][model]['ROC-EOD'][metric]['mean'], label = "ROC-EOD", linestyle="--", marker="^", markeredgewidth=0, c=colors['ROC-EOD'], alpha=1)
...
def plot_bargraph(retrieval_path:str, dataset_list=['student','OULADstem', 'OULADsocial'], metric_list=['acc','StatParity','EqqOddsDiff','GenEntropyIndex'],
    ...
    """
    dataset_list=['student','OULADstem', 'OULADsocial']
    ...
    for ds in dataset_list :
        ...
        for metric in metric_list :
            for bias in bias_list :
                ...
                ax.bar(indices - 3.5*bar_width,metrics_all[ds][bias][''][metric]['mean'], label = 'biased model', width=bar_width,color=colors[''])#Black
                ...
                #Preprocessingf
                ax.bar(indices - 2.5*bar_width,metrics_all[ds][bias]['reweighting'][metric]['mean'], label = 'reweighing', width=bar_width, color=colors['reweighting'])#Tenne/Dark orange
                ax.bar(indices - 1.5*bar_width,metrics_all[ds][bias]['massaging'][metric]['mean'], label = 'massaging', width=bar_width, color=colors['massaging'])#Pumpkin/Bright orange
                ax.bar(indices - 0.5*bar_width,metrics_all[ds][bias]['FTU'][metric]['mean'], label = 'FTU', width=bar_width, color=colors['FTU'])#Mac and cheese orange
                ...
                ax.bar(indices + 0.5*bar_width,metrics_all[ds][bias]['eqOddsProc'][metric]['mean'], label = 'EOP', width=bar_width, color=colors['eqOddsProc'])#Cerulean/Blue
                ...
                ax.bar(indices + 1.5*bar_width,metrics_all[ds][bias]['calEqOddsProc'][metric]['mean'], label = 'CEO', width=bar_width, color=colors['calEqOddsProc'])#Picton blue
                ...
                ax.bar(indices + 2.5*bar_width,metrics_all[ds][bias]['ROC-SPD'][metric]['mean'], label = 'ROC-SPD', width=bar_width, color=colors['ROC-SPD'])#Seil/Light blue
                ...
                ax.bar(indices + 3.5*bar_width,metrics_all[ds][bias]['ROC-EOD'][metric]['mean'], label = 'ROC-EOD', width=bar_width, color=colors['ROC-EOD'])#Dark Grey
                
                if plot_style == 'FILLED_STDEV':
                    ax.errorbar(indices - 3.5*bar_width, metrics_all[ds][bias][''][metric]['mean'], yerr=metrics_all[ds][bias][''][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors[''])
                    ax.errorbar(indices - 2.5*bar_width, metrics_all[ds][bias]['reweighting'][metric]['mean'], yerr=metrics_all[ds][bias]['reweighting'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['reweighting'])
                    ax.errorbar(indices - 1.5*bar_width, metrics_all[ds][bias]['massaging'][metric]['mean'], yerr=metrics_all[ds][bias]['massaging'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['massaging'])
                    ax.errorbar(indices - 0.5*bar_width, metrics_all[ds][bias]['FTU'][metric]['mean'], yerr=metrics_all[ds][bias]['FTU'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['FTU'])
                    ax.errorbar(indices + 0.5*bar_width, metrics_all[ds][bias]['eqOddsProc'][metric]['mean'], yerr=metrics_all[ds][bias]['eqOddsProc'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['eqOddsProc'])
                    ax.errorbar(indices + 1.5*bar_width, metrics_all[ds][bias]['calEqOddsProc'][metric]['mean'], yerr=metrics_all[ds][bias]['calEqOddsProc'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['calEqOddsProc'])
                    ax.errorbar(indices + 2.5*bar_width, metrics_all[ds][bias]['ROC-SPD'][metric]['mean'], yerr=metrics_all[ds][bias]['ROC-SPD'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['ROC-SPD'])
                    ax.errorbar(indices + 3.5*bar_width, metrics_all[ds][bias]['ROC-EOD'][metric]['mean'], yerr=metrics_all[ds][bias]['ROC-EOD'][metric]['stdev'], fmt = 'none', elinewidth=0.5, capsize=1, capthick=0.5, color=colors['ROC-EOD'])
                ...
                if ylim is not None:
                    ...
                    if metric == 'acc':
                        ...
                        if ds != 'student':
                            ax.set_ylim(0.0,0.25) #,auto=False)
                            #ax.set_ybound([0.0,0.5])
                    elif metric == 'StatParity':
                        if bias == 'label':
                             ax.set_ylim(-0.82,0.25)
                        elif bias == 'selectDoubleProp':
                            ax.set_ylim(-0.61,0.41)
                        elif bias == 'selectLow':
...
def bias_vs_unbiased(retrieval_path:str, bias_type, dataset_list:list[str], model_list=list[str], preproc_list=['', 'reweighting','massaging'],
    ...
    #ax.grid(visible=True)
    if bias_type == 'label':
        ax.plot([-1, 1], [-1, 1], linestyle=':', color='black', zorder=1)
    elif bias_type == 'selectDoubleProp':
        ax.plot([-1, 3.5], [-1, 3.5], linestyle=':', color='black', zorder=1)
    elif bias_type == 'selectLow':
        ax.plot([-0.25, 3], [-0.25, 3], linestyle=':', color='black', zorder=1)
    ...
    colorblindIBM = {'ultramarine': "#648FFF", 'indigo':"785EF0", 'magenta':"#DC267F", 'orange':"#FE6100", 'gold':"#FFB000",
                 'black':"#000000", 'white':"#FFFFFF"}
    m_colors = {'acc':"#785EF0", 'BlindCons':"#DC267F", 'StatParity': "#FE6100", 'EqqOddsDiff': "#FFB000", 'GenEntropyIndex':"#648FFF"}
    m_legend = {'StatParity': "SPD", 'EqqOddsDiff': "EOD", 'acc':"Accuracy", 'BlindCons':"Consistency", 'GenEntropyIndex':"GEI"} 
    m_marker = {'acc':"o", 'BlindCons':"p", 'StatParity': "d", 'EqqOddsDiff': "P", 'GenEntropyIndex':"p"}
    alpha = 0.4
    label = True
    for ds in dataset_list:
